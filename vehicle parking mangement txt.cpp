...
void EarnedMoney:: getAllEarnedMoney(){
    ifstream fin;
    fin.open("aaEarnedMonry.txt",ios::in);
    if(!fin){
        cout<<"\nFile Not Found";
    }
    else{
...
class Vehicle:public VehicleType{
    ...
    public:
        Vehicle(){
            ...
            strcpy(this->createdBy,"Mohit ");
            strcpy(this->lastUpdatedBy,"Sharma");
            time_t tt;
            time(&tt);
        ...
        void setVehicle(){
            ...
            strcpy(createdBy,"mohit");
            strcpy(lastUpdatedBy,"sharma");
...
void Vehicle:: addVehicle(){
    ofstream fout;
    fout.open("aaVehivleData.txt",ios::app);
    //getch();
    ...
    fout.open("aaEarnedMonry.txt",ios::out);
...
void Vehicle:: getAllVehicleList(){
    ifstream fin;
    int nor=0;
    fin.open("aaVehivleData.txt",ios::in);
    if(!fin){
        cout<<"\nFile Not Found";
    }
    else{
...
void Vehicle:: deleteVehicle(int vno){
    ...
    if(!fin){
        ...
        fin.read((char*)this,sizeof(*this));
        fout.open("aaTempVehivleData.txt",ios::out);
        while(!fin.eof()){
            if(this->vehicleNo==vno){
                flag=1;
            }
...
void Vehicle:: updateVehicle(int vno){
    fstream foutIn;
    int flag=0;
    foutIn.open("aaVehivleData.txt",ios::in|ios::out|ios::ate);
    foutIn.seekg(0);
    if(!foutIn){
        cout<<"\nFile Not Found";
    }
